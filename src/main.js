...
async function onWalletConnected(address) {
  ...
  try {
    ...
    // PERBAIKAN: Pakai custom fullnode Shelbynet, bukan Network.TESTNET
    ...
    try {
      ...
      log('APT SDK err: ' + e.message + ' → fallback REST', 'in');
      // Fallback: fetch manual via REST jika SDK gagal
      try {
        const res = await fetch(
          `${SHELBY_CONFIG.aptosFullnode}/accounts/${address}/resource/0x1::coin::CoinStore<0x1::aptos_coin::AptosCoin>`
        );
        if (res.ok) {
          const data = await res.json();
          const octa = Number(data?.data?.coin?.value || 0);
          aptBal = (octa / 1e8).toFixed(4);
          log('APT · ' + aptBal + ' (REST fallback)', 'ok');
        }
      } catch(e2) { log('APT REST err: ' + e2.message, 'in'); }
    }
